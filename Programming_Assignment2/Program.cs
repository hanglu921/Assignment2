    ...
    class Program
    {
        ...
        private static int LuckyNumber(int[] nums)
        {
            try
            {
                ...
                return list.Count == 0 ? -1 : list.Max();
            }
            catch (Exception)
            {
        ...
        private static int MinCostToClimb(int[] costs)
        {
            try
            {
                ...
                return Math.Min(Min(costs.Length - 1), Min(costs.Length - 2));

                int Min(int i)
                {
                    if (memo.TryGetValue(i, out var minCost))
                        return minCost;

                    var result = costs[i];
                    if (i > 1)
                        result += Math.Min(Min(i - 1), Min(i - 2));
